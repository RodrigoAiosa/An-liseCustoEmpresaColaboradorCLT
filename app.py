...
with st.sidebar:
    ...
    with st.expander("📌 Dados Contratuais", expanded=True):
        ...
        regime = st.selectbox("Regime de Contratação",
    ...
    with st.expander("💻 Outros Custos"):
        aux_home = st.number_input("Auxílio Home Office", value=0.0)
        epi_equip = st.number_input("EPI / Notebook (Mensalizado)", value=0.0)
        outros_c = st.number_input("Outras Taxas", value=0.0)
